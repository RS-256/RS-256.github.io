...
export function fetchDiscordInvite( url: string ): Promise< DiscordInviteInfo | null > {
...
async function fetchInviteInfo( code: string ): Promise< DiscordInviteInfo | null > {
  try {
    const res = await fetch( `https://discord.com/api/v10/invites/${ code }?with_counts=true` )
    if ( ! res.ok ) {
      console.warn( `[discord] 招待コード ${ code } の取得に失敗 (HTTP ${ res.status })。手書きラベルで表示します。` )
      return null
    }
    ...
    return {
      name: guild.name,
      iconUrl: guild.icon ? `https://cdn.discordapp.com/icons/${ guild.id }/${ guild.icon }.png?size=96` : undefined,
